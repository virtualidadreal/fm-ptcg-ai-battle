"""
Sabrina v2 — Alakazam (Abra->Kadabra->Alakazam, Powerful Hand) rule-based agent.

v2 = the top-pilot deck (11 list changes vs v1, see research/alakazam-top-pilot-analisis-2026-06-22.md)
piloted in, plus the A1 code change. Summary of what changed in this file vs the verbatim v1 fork:
  - A1 (verified-safe): decline the optional Psychic Draw on an ACTIVATE YES prompt when we're
    already winning AND the deck is low (_deck_preserve) -> stop drawing ourselves out of a won game.
  - Dunsparce 305: v2 ships id305 (Trading Places 423 / Ram 424), NOT id65. C.DUNSPARCE re-keyed
    65 -> 305 so the whole Dunsparce engine (opening / bench / to-hand / draw) actually pilots the
    real card. 305 -> Dudunsparce 66 confirmed by recon. (id65 logic is now inert/harmless.)
  - Fezandipiti ex (140, NEW): 2-PRIZE liability flown as a bench-only draw engine (Flip the Script)
    with 6 guards — never opened, never promoted, never energised, only its ability fires to close
    lethal / fix setup, and it's protected from put-back.
  - Lillie's Determination (1227, NEW x4): reset-draw supporter, gated on the supporter slot +
    _deck_preserve, strongest while prizes==6 (draws 8).
  - Xerosic's Machinations (1197, NEW x1): hand-disruption supporter, ranked BELOW our own draw
    engines, dead when opp hand<=3.
  - QUITADAS now inert (harmless, never fire under the v2 list): Dunsparce id65 (Gnaw/Dig),
    Battle Cage 1264, Hero's Cape 1159.
...
class C:
    ...
    DUNSPARCE = 305       # v2 deck ships id305 (Trading Places 423 / Ram 424, 70HP, retreat 1).
                          # id65 (Gnaw 74 / Dig 75) is the QUITADA print — NOT in the v2 deck, so any
                          # code keyed on 65 is now inert. Recon confirms 305 -> Dudunsparce 66 works
                          # identically to 65 -> 66 (engine matches evolvesFrom by name 'Dunsparce').
    DUDUNSPARCE = 66      # Stage1 draw engine (Run Away Draw). v2 engine room: -1 copy -> 66:3.
    FEZANDIPITI = 140     # NEW: Basic Darkness ex (2 prizes!), bench draw-engine via Flip the Script.
                          # NEVER open/promote it (2-prize liability). Bench-only support body.
    ...
    LILLIE_DET = 1227     # NEW Supporter: shuffle hand, draw 6 (8 if exactly 6 prizes). Reset draw.
    XEROSIC = 1197        # NEW Supporter: opp discards down to 3 cards. Hand disruption.
    RARE_CANDY = 1079
    BOSS_ORDERS = 1182
...
DUNSPARCE_TRADE = 423  # Dunsparce 305: Trading Places (free switch). Recon: 423/424 belong to id305.
DUNSPARCE_RAM = 424    # Dunsparce 305: Ram (CC, 20 dmg)
...
class AlakazamPolicy:
    ...
    def _score(self, o):
        ...
        # A1 (verified): decline the optional Psychic Draw (Kadabra-on-evolve / any ACTIVATE
        # YES prompt) when we're already winning and the deck is low — drawing more here only
        # risks decking out before the last prize. _deck_preserve() already requires a powered
        # attacker + KO-capable hand, so saying NO costs us nothing and saves the deck.
        # NB: must score YES strictly BELOW NO (NO returns 0); a tie at 0 would still pick YES
        # (normalize fills minCount with the highest-ranked option, and YES sorts first on a tie),
        # so the decline only actually fires if YES is negative.
        if self.context == SelectContext.ACTIVATE and t == OptionType.YES and self._deck_preserve():
            return -1
        if t == OptionType.YES:
            return 1
        if t == OptionType.NO:
    ...
    def _score_ability(self, o):
        ...
        if card.id == C.FEZANDIPITI:
            # GUARD (e): Flip the Script (draw 3 after one of our Pokemon was KO'd last opp
            # turn, once/turn). The engine only offers it when its KO condition is already met,
            # so use it ONLY when the extra 3 cards close a lethal or repair the setup —
            # otherwise it just burns deck (deck-out brake). Reuse existing predicates.
            if self.me.deckCount <= 7:
                return -1
            if self._deck_preserve():
                return -1
            if self._ko_active_reachable() or self._need_pieces():
                return 14500
            return -1
        return 9000
    ...
    def _score_play_poke(self, card):
        ...
        if cid == C.FEZANDIPITI:
            # GUARD (d): bench-only draw engine (Flip the Script: draw 3 if one of our
            # Pokemon was KO'd last opp turn). Worth benching as a recovery engine, but well
            # below the real line bodies (it's a 2-prize liability that must stay benched).
            return 7000 if n == 0 else -1
        if cid == C.SHAYMIN:
    ...
    def _score_play_trainer(self, card):
        ...
        if cid in (C.HILDA, C.DAWN, C.POKE_PAD, C.LILLIE_DET) and self._deck_preserve():
            return -1
        if cid == C.LILLIE_DET:
            # NEW reset-draw supporter (shuffle hand, draw 6; 8 while prizes==6). Highest-impact
            # deck change (arms Powerful Hand by refilling a clogged/small hand). Gated on the
            # supporter slot + _deck_preserve (above), NOT hard-gated on hand size (H-4 = hypothesis,
            # not law). Strongest before any prize is taken (draws 8). Ranks alongside Hilda/Dawn.
            if self.state.supporterPlayed:
                return -1
            if draw_for_ko:
                return 14200
            prizes_full = len(self.me.prize) == 6
            base = 12800 if prizes_full else 12300   # the +8-card turn-1 reset is the best line
            return base if (self._need_pieces() or self.me.handCount <= 4) else 2800
        if cid == C.XEROSIC:
            # NEW disruption supporter (opp discards down to 3). Situational, NOT the draw engine:
            # dead if opp hand <= 3, scales with opp handCount, and it competes for the 1 supporter/
            # turn against our own draw/setup. So rank it BELOW the draw supporters in normal lines,
            # only worth the slot when the opponent is holding a big setup hand and we don't need to
            # draw toward a KO ourselves.
            if self.state.supporterPlayed:
                return -1
            opp_hand = self.opponent.handCount
            if opp_hand <= 3:
                return -1                       # no cards stripped -> dead play
            if draw_for_ko or self._energy_starved() or self._need_pieces():
                return 1200                     # our own setup/draw beats disruption here
            if opp_hand >= 6:
                return 11500                    # strip a loaded setup hand (still below draw engines)
            return 4000
        if cid == C.HILDA:
            if self.state.supporterPlayed:
                return -1
    ...
    def _score_active_choice(self, o, card):
        ...
        if card.id in ALAKAZAM_IDS:
            ...
            # GUARD (c): NEVER promote the 2-prize ex into the active slot after a KO —
            # it gives the opponent 2 free prizes and can't attack our line. Hard-penalize.
            score -= 200
    ...
    def _score_setup_active(self, card):
        ...
        if card.id == C.FEZANDIPITI:
            # GUARD (b): a 2-PRIZE ex with no attacker line — opening it = catastrophe
            # (free 2 prizes if KO'd, can't pressure). Last resort, below every real body.
            return 0
    ...
    def _score_to_bench(self, card):
        ...
        if cid == C.DUNSPARCE:
            return 180 - 30 * n
        if cid == C.FEZANDIPITI:
            # GUARD (d): bench it as the draw engine, single copy, never duplicate.
            return 70 if n == 0 else -1
        if cid == C.SHAYMIN:
    ...
    def _score_putback(self, card):
        ...
        if self.hand[card.id] >= 2:
            return 60
        if card.id in (C.ABRA, C.ALAKAZAM, C.DUNSPARCE, C.FEZANDIPITI):
            return -40            # GUARD (f): don't bury the single-copy draw engine / line pieces
        return 10
