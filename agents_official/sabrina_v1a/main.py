"""
Sabrina v1a — Alakazam (Abra->Kadabra->Alakazam, Powerful Hand) rule-based agent.

Sabrina v1a = Sabrina v1 + 4x Lillie's Determination (id 1227) cutting exactly 4x Battle
Cage (id 1264). SINGLE-VARIABLE A/B of the REAL ladder vs v1 (v1 = 826.9 = our floor).
Thesis (DOMAIN, not cabt): import the top-pilot's hand-refill engine (Lillie's
Determination: discard hand, draw 6 / 8 at 6 prizes) — a pure upgrade to the motor that
powers Powerful Hand (20 dmg x card in hand) — at the cost of the static defense Stadium
(Battle Cage) that the single-prize prize-race plan never leverages (the #1 ladder
Alakazam runs 0 Battle Cage and 4x Lillie's Determination). No code change: 1227 is a
draw Supporter with no dedicated branch, so it falls through _score_play_trainer's generic
`return 9000` and is piloted as a sane sub-Rare-Candy/Hilda/Dawn, above-END draw play; the
engine only ever offers it as a LEGAL option (supporterPlayed is enforced engine-side), so
0-crash/0-INVALID. The A1 code change (decline Psychic Draw under deck-preserve) is NOT in
v1a — kept as a separate A/B. Fezandipiti ex (140) is the suspect-1 isolate for the NEXT
A/B (2-prize body in a single-prize prize-race deck).
