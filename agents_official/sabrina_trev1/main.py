...
class AlakazamPolicy:
    ...
    def _deck_preserve(self):
        ...
        deck_low = self.me.deckCount <= remaining_prizes + 8   # keep a draw-1/turn buffer (sabrina_trev1: was +4, widened to +8). Anti-deckout vs slow grind walls like Hop's Trevenant, where we mill to 0 with the rival still on prizes (4/5 top-pilot losses were deckouts; +8 absorbs the worst single heavy-draw turn ~8 cards, our deck mills ~33% slower than the data source so +8 is well-cushioned). remaining_prizes = len(self.me.prize) = OUR prizes still to take (verified). One-variable A/B vs sabrina_v1; cabt is anti-predictive (-0.80) so a cabt drop must NOT veto this, judge on deck-out rate + ladder WR split Trevenant/non-Trevenant.
