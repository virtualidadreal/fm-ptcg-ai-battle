...
ATTACK_DAMAGE = {}            # A1: static base damage per attackId (0 for scaling/effect attacks)
...
for _a in all_attack():
    ...
    ATTACK_DAMAGE[_a.attackId] = getattr(_a, 'damage', 0) or 0   # A1 net-prize-trade
    _t = (_a.text or '').lower()
    if 'for each' in _t and 'energy attached to this' in _t:
        SELF_SCALING_ATTACKS.add(_a.attackId)

# ── A1 net-prize-trade config ────────────────────────────────────────────────
# A1 net-prize-trade: penalize an attack that LEAVES our own Active exposed to a KO
# next turn WHEN that Active is worth >1 prize (an ex/Mega). Isolated behind a flag so
# the A/B vs v3 has exactly one variable. Default ON in this variant.
FMA_A1_NETPRIZE = os.environ.get("FMA_A1_NETPRIZE", "1") not in ("0", "false", "False", "")
# lambda: cost (in score points) per prize the opponent would gain by KO'ing our exposed
# Active. _score_attack damage scores live on a ~1000 + min(dmg,320) scale (so a normal
# attack scores ~1000-1320, and a same-turn lethal jumps to 90000). We set lambda so that
# the A1 penalty for a 2-prize exposure is meaningful but NEVER overrides a winning/lethal
# play (those score 90000/+2500): 600 per prize => 1200 for a 2-prize body. That can flip
# the choice between two non-lethal attacks (or attack-vs-pass on the margin) without ever
# vetoing a KO. Value is deliberately conservative on first cut; tune via A/B, not locally.
A1_LAMBDA = float(os.environ.get("FMA_A1_LAMBDA", "600"))
...
class AlakazamPolicy:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # A1 net-prize-trade — the single isolated edge vs v3.
    #   Conservative estimate of the MOST damage the opponent's current Active could
    #   deal to OUR Active next turn, using the static per-attack base damage from the
    #   engine card data (ATTACK_DAMAGE), adjusted for our Active's weakness/resistance.
    #   It is intentionally conservative / under-counting:
    #     - only the opp ACTIVE is considered (a benched attacker would need to retreat/gust
    #       first; we do not assume the opp will set that up),
    #     - scaling / effect-based attacks (Powerful Hand, "X per energy", counter-placement)
    #       carry damage=0 in the static table, so they are treated as 0 here. This means we
    #       NEVER over-estimate the opponent (we may under-estimate, i.e. fail to penalize),
    #       which keeps A1 from vetoing good plays on a guess.
    #   Returns an int of estimated max incoming damage (0 if unknown).
    # ═══════════════════════════════════════════════════════════════════════
    def _opp_max_next_damage(self):
        opp = self.opponent.active[0] if self.opponent.active else None
        my_active = self.me.active[0] if self.me.active else None
        ...
        if best <= 0:
            return 0
        # weakness / resistance vs OUR active's type (engine doubles on weakness, -30 on resist)
        md = card_table.get(my_active.id)
        opp_type = getattr(oc, 'energyType', None)
        if md is not None and opp_type is not None:
            if md.weakness == opp_type:
                best *= 2
            elif md.resistance == opp_type:
                best = max(0, best - 30)
        return best

    def _a1_self_exposure_penalty(self):
        """A1 net-prize-trade penalty (>= 0). Fires only when, after attacking, our OWN
        Active (a) is worth >1 prize AND (b) is left in range of being KO'd by the opp's
        Active next turn. Penalty = A1_LAMBDA * prize_yield(our active). Subtracted from the
        attack score so we prefer not to expose a multi-prize body on an unfavorable trade."""
        if not FMA_A1_NETPRIZE:
            return 0.0
        my_active = self.me.active[0] if self.me.active else None
        if my_active is None:
            return 0.0
        prize_yield = prize_count(my_active)        # 1 normal, 2 ex, 3 mega-ex
        if prize_yield <= 1:
            return 0.0                              # only multi-prize bodies are worth protecting
        # our Active does not take damage on OUR turn, so its HP next turn ~= its current hp
        my_hp_after = getattr(my_active, 'hp', 0)
        incoming = self._opp_max_next_damage()
        if incoming <= 0 or incoming < my_hp_after:
            return 0.0                              # not in KO range (conservatively estimated)
        return A1_LAMBDA * prize_yield
    ...
    def _score_attack(self, o):
        ...
        # A1 net-prize-trade: subtract lambda * prize_yield when this attack leaves OUR
        # multi-prize Active exposed to a KO next turn. Skipped when this very attack KOs
        # the opp (own_ko): if we are already taking a prize this turn, the trade is not the
        # unfavorable one A1 targets. Never applied to the 90000 winning path above.
        if not own_ko:
            score -= self._a1_self_exposure_penalty()
        return score
