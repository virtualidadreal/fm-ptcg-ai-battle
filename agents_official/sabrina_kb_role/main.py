"""
Sabrina kb_role — Sabrina v1 + ONE lever: dynamic BEATDOWN/CONTROL role assignment.

Base: an EXACT copy of agents_official/sabrina_v1 (the verbatim AlakazamPolicy + the FMA
hardening scaffolding). deck.csv is byte-identical to sabrina_v1 (this is a 1-lever change =
pure logic; the list does NOT change).

THE ONE LEVER (archetype-agnostic, bounded, reversible — gated behind a flag):
  A small, generic `RoleAssigner` reads only PUBLIC turn state (prize_diff + a speed proxy:
  can-I-OHKO-the-active vs can-the-opponent-OHKO-me) and labels the turn BEATDOWN or CONTROL.
  Then a thin post-hoc NUDGE re-weights a handful of already-existing scores by a small,
  bounded factor:
    - CONTROL  -> don't deck yourself out (raise the deck-preserve / draw-restraint bar) and
                 favour attrition; never chase a non-lethal over-commit.
    - BEATDOWN -> bias toward the close (lethal attacks, gusting the threat, drawing toward a
                 lethal Powerful Hand).
  The nudge is a MULTIPLIER on existing scores (never a hard override, never a new legal move),
  so the base policy's choice still wins whenever it is far ahead — the role only breaks ties /
  tilts close calls. Disable with env SABRINA_ROLE_NUDGE=0 (then this file == sabrina_v1).

PROVENANCE OF THE LEVER: knowledge/synthesis/heuristicas-computables.md #8 ("Asignación
dinámica de rol BEATDOWN vs CONTROL", from the `archetypes` fiche). Per that doc's caveat,
the velocity/favorability functions in the fiche are STUBS and one fiche fabricated a Flipside
quote — so we DO NOT import its `assign_role` verbatim. We implement only the parts that are
INVARIANTS of imperfect-info Rule-Box mechanics and computable from public state in cabt
(prize_diff is public; OHKO-reachability we already compute via _alakazam_damage/_achievable_hand).
`matchup_fav` and hidden-energy speed estimates (the rotating/meta-dependent parts) are NOT used.

ARCHETYPE-AGNOSTIC DESIGN GOAL: `RoleAssigner` takes a tiny duck-typed `RoleInputs` struct
(prize_diff, can_ohko_active, opp_can_ohko_me) — it knows nothing about Alakazam. Any future
pilot (Dragapult/Leon, a general "best PTCG player" core) can feed the same struct and reuse
the module unchanged. Sabrina only supplies the adapter that fills RoleInputs from its policy.

HONEST BASELINE (inherited): ptcg-abc's Alakazam scored ~674 on the real ladder; sabrina_v1 is
the validated FLOOR of the line. cabt is anti-predictive of the ladder (Spearman ~ -0.80) — a
cabt dip is NOT evidence the lever is bad. The ladder is the only judge; A/B vs sabrina_v1.
...
# ── THE LEVER: archetype-agnostic dynamic role (BEATDOWN / CONTROL) ──────────
# Flag (default ON). Set SABRINA_ROLE_NUDGE=0 to turn the lever off entirely → this
# file behaves EXACTLY like sabrina_v1 (the reversible/ablation switch for the A/B).
...
ROLE_NUDGE_ON = _flag_on("SABRINA_ROLE_NUDGE", True)

# Bounded, reversible multipliers. These are NUDGES (close to 1.0), never overrides:
# the base policy already separates classes by orders of magnitude (attacks ~1e3-9e4,
# trainers ~1e4, draw abilities ~1.5e4), so a ±15-25% tilt only re-orders NEAR-TIES
# inside a class and never promotes a fundamentally worse move over a better one.
# Lethal/game-winning scores (the 90000 band) are deliberately left untouched in both
# roles — you always take the game-winning line regardless of role.
ROLE_NUDGE = {
    "BEATDOWN": {"close": 1.20, "draw": 1.00, "preserve_bar": 0},   # bias the kill
    "CONTROL":  {"close": 1.00, "draw": 0.80, "preserve_bar": 4},   # don't deck out; attrition
    "NEUTRAL":  {"close": 1.00, "draw": 1.00, "preserve_bar": 0},
}


class RoleInputs:
    """Tiny duck-typed, ARCHETYPE-AGNOSTIC input struct for the role assigner.
    Knows nothing about Alakazam — any pilot can fill it from public turn state.
      prize_diff       = opp_prizes_remaining - my_prizes_remaining  (>0 ⇒ I'm AHEAD)
      can_ohko_active  = can I KO the opponent's active THIS turn (incl. reachable draws)?
      opp_can_ohko_me  = can the opponent KO my active next turn with visible resources?
    """
    __slots__ = ("prize_diff", "can_ohko_active", "opp_can_ohko_me")

    def __init__(self, prize_diff, can_ohko_active, opp_can_ohko_me):
        self.prize_diff = prize_diff
        self.can_ohko_active = bool(can_ohko_active)
        self.opp_can_ohko_me = bool(opp_can_ohko_me)


def assign_role(ri):
    """Heuristics-computables #8, reduced to the meta-INVARIANT, public-state core.
    Returns 'BEATDOWN' | 'CONTROL' | 'NEUTRAL'. No matchup_fav / hidden-speed stubs.

      ahead + can close            -> BEATDOWN (press the lead to the finish)
      they can KO me, I can't them  -> CONTROL  (can't win the race: attrition / no deck-out)
      behind by >1 prize            -> CONTROL  (grind back, don't trade into a lost race)
      else                          -> NEUTRAL  (let the base policy decide untouched)
    """
    if ri.prize_diff > 0 and ri.can_ohko_active:
        return "BEATDOWN"
    if ri.opp_can_ohko_me and not ri.can_ohko_active:
        return "CONTROL"
    if ri.prize_diff < -1:
        return "CONTROL"
    return "NEUTRAL"
...
_attack_table = {}               # attackId -> attack object (for the role speed proxy)
for _a in all_attack():
    _attack_table[_a.attackId] = _a
...
class AlakazamPolicy:
    ...
    # — THE LEVER: adapter filling the archetype-agnostic RoleInputs from public state —
    def _opp_can_ohko_me(self):
        """Speed proxy (visible-resource only): can the opponent's active KO my active
        next turn with the energy ALREADY attached to it? Public state only — no hidden
        belief. Type-aware via _can_pay; uses the engine's attack damage table best-effort."""
        my_a = self.me.active[0] if self.me.active else None
        opp = self.opponent.active[0] if self.opponent.active else None
        if my_a is None or opp is None:
            return False
        my_hp_left = max(0, getattr(my_a, "hp", 0) - getattr(my_a, "damage", 0))
        if my_hp_left <= 0:
            return False
        ...
        for aid in (oc.attacks or []):
            ...
            if cost is None or not self._can_pay(attached, cost):
                continue
            dmg = getattr(_attack_table.get(aid), "damage", 0) or 0
            mine = card_table.get(my_a.id)
            if mine is not None and dmg:
                if mine.weakness == getattr(oc, "type", None):
                    dmg *= 2
                elif mine.resistance == getattr(oc, "type", None):
                    dmg = max(0, dmg - 30)
            best = max(best, dmg)
        return best >= my_hp_left

    def role(self):
        """Cached per-policy role label. NEUTRAL when the lever is off (== sabrina_v1)."""
        if not ROLE_NUDGE_ON:
            return "NEUTRAL"
        cached = getattr(self, "_role_cache", None)
        if cached is not None:
            return cached
        opp = self.opponent.active[0] if self.opponent.active else None
        prize_diff = len(self.opponent.prize) - len(self.me.prize)   # >0 ⇒ I'm ahead
        can_ohko = bool(opp is not None and self._ko_active_reachable())
        opp_ohko = self._opp_can_ohko_me()
        r = assign_role(RoleInputs(prize_diff, can_ohko, opp_ohko))
        self._role_cache = r
        return r

    def _role_mul(self, key):
        return ROLE_NUDGE.get(self.role(), ROLE_NUDGE["NEUTRAL"]).get(key, 1.0)
    ...
    def _deck_preserve(self):
        ...
        # LEVER: in CONTROL, widen the deck-preserve buffer (don't mill into a deck-out while
        # grinding an attrition game). +0 in BEATDOWN/NEUTRAL ⇒ identical to sabrina_v1.
        buffer = 4 + ROLE_NUDGE.get(self.role(), ROLE_NUDGE["NEUTRAL"]).get("preserve_bar", 0)
        deck_low = self.me.deckCount <= remaining_prizes + buffer  # keep a draw-1/turn buffer
    ...
    def _score_ability(self, o):
        ...
        if card.id == C.DUDUNSPARCE:
            ...
            if self.me.handCount >= 14 and self.me.deckCount <= 12:
                return -1
            # LEVER (bounded): in CONTROL, gently restrain the aggressive draw engine when the
            # deck is genuinely getting low (attrition mode prefers not to mill toward a deck-out
            # while grinding). The 0.80 factor only bites once the deck is short — above the
            # generic-ability floor (9000) it stays the top draw, so we never STOP drawing, we
            # only let it lose ties to non-mill lines late. In BEATDOWN/NEUTRAL: ×1.0 == sabrina_v1.
            base = 15000
            if self.me.deckCount <= LOW_DECK_COUNT + 6:
                base = int(base * self._role_mul("draw"))
            return base
        return 9000
    ...
    def _score_attack(self, o):
        ...
        if opp.hp <= dmg and prize_count(opp) >= len(self.me.prize):
            return 90000        # game-winning line is role-independent (never nudged)
        score = 1000 + min(dmg, 320)
        if opp.hp <= dmg:
            score += 2500 + prize_count(opp) * 200
        # LEVER (bounded): in BEATDOWN, bias toward the close — value a real KO/damage swing
        # a little higher so a tie between "attack now" and a reversible setup tilts to attack.
        # The factor (1.20 BEATDOWN / 1.00 otherwise) only re-orders near-ties; END=0 and the
        # switch/utility floors below ~700 stay below any real attack, so no stall is introduced.
        return score * self._role_mul("close")
