...
class C:
    ...
    DUNSPARCE = 65        # canonical Dunsparce id used by ALL scoring/counter logic
    DUNSPARCE_305 = 305   # the OTHER Dunsparce printing (Trading Places, 70HP, free pivot of
                          # the active). Deck now ships 4x 305 instead of 4x 65. Behaviourally it
                          # is the SAME evolution piece (-> Dudunsparce 66) so it is canonicalised
                          # to DUNSPARCE everywhere: identical scoring, only the printing changed.
...
DUNSPARCE_PRINTINGS = {C.DUNSPARCE, C.DUNSPARCE_305}  # both printings are the same evo piece

def canon_id(cid):
    # Fold the alternate Dunsparce printing (305) onto the canonical Dunsparce id (65) so every
    # existing C.DUNSPARCE comparison / field-hand-discard counter treats them as one card.
    return C.DUNSPARCE if cid == C.DUNSPARCE_305 else cid
...
class AlakazamPolicy:
    def __init__(self, obs: Observation):
        ...
        for p in self._my_board():
            if p is not None:
                self.field[canon_id(p.id)] += 1
        for c in self.me.hand:
            self.hand[canon_id(c.id)] += 1
        for c in self.me.discard:
            self.discard[canon_id(c.id)] += 1
    ...
    def _score_play_poke(self, card):
        cid = canon_id(card.id); n = self.field[cid]
        if cid == C.ABRA:
            return 20000 - 250 * n
        if cid == C.DUNSPARCE:
            return 18500 - 250 * n
    ...
    def _score_putback(self, card):
        if card is None:
            return 0
        pcid = canon_id(card.id)
        if self.hand[pcid] >= 2:
            return 60
        if pcid in (C.ABRA, C.ALAKAZAM, C.DUNSPARCE):
            return -40
        return 10
