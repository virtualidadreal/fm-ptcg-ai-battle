"""
Sabrina cons — Alakazam (Abra->Kadabra->Alakazam, Powerful Hand) rule-based agent.

CONSISTENCY FORK of Sabrina v1 (best line so far, 826.9). ONE variable changed, nothing
else: deck.csv only. The PILOTING CODE IS BYTE-FOR-BYTE IDENTICAL to v1 (verified: only
deck.csv differs). The single lever is the cheapest and most ladder-real one per the
LB950 "strong-start baseline v10" notebook (Mega Lucario ex): raise the count of BASIC
Pokemon so the opening hand bricks less. v1 ran only 8 basics in 60 (Abra 741 x4 +
Dunsparce 65 x4 — everything else is evolutions/energy/trainers, which don't count for
mulligan), the WORST rung on the basics ladder: P(0 basics in opening 7) =
C(52,7)/C(60,7) = 34.64% — one in three openings is a mulligan, gifting the rival a card
and starting a slow Stage-2 line down tempo. This fork goes to 12 basics (LB950's own
validated target: 19.1%) by adding Shaymin 343 x4 and cutting four marginal tech
SINGLETONS (Hero's Cape 1159, Wondrous Patch 1146, Sacred Ash 1129, Lana's Aid 1184),
keeping Night Stretcher 1097 as the one flexible recursion. New mulligan (hypergeometric,
60 cards, 7-card hand) = C(48,7)/C(60,7) = 19.06%, a -15.58 pp absolute drop, nearly half
the mulligans removed. Ladder mechanics make this pay fast: TrueSkill mu-3sigma, only
W/L/D (margin doesn't score) — an honest consistency gain compounds quickly. Shaymin 343 is
a non-ex basic the pilot ALREADY handles correctly (never opened active over Abra; benched
only when _opp_threatens_bench via Flower Curtain), so it adds basics for mulligan WITHOUT
clogging the bench like a generically-scored body (Fan Rotom) would; 0-crash via the existing
fallbacks (benches it, never strands it as Active over Abra — no new code needed). One
variable, deck only, ladder is the sole judge.
