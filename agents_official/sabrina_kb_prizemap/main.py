"""
Sabrina kb_prizemap — Sabrina v1 + ONE lever: adaptive 6-prize PRIZE-MAP (Knowledge OS H1b).

================================ THE ONE LEVER ================================
A single-variable, flag-gated, ARCHETYPE-AGNOSTIC fork of `sabrina_v1`. The ONLY behavioural
change is a thin post-hoc NUDGE that biases ATTACK and gust-TARGET selection to stay on the
CHEAPEST KO sequence that closes out the remaining prizes — the "prize map" upgrade (H1b) from
the CFB Edge / Isaiah Bradner video (knowledge/free/video-enrichment-2026-06-24.md §1).

H1b is "plan the 6 prizes, recompute every turn, pick the play that keeps the map on track,
GATE out plans without the resources, and deck-out-check before committing". `sabrina_kb_draw`
already implements the deck-out-guard HALF (stop optional drawing when the win-con is safe).
THIS lever is the OTHER half: the adaptive prize PLAN that biases attack/target selection so we
spend our KOs on the cheapest reachable sequence to the last prize, and a feasibility GATE that
withholds the bonus from a target whose prize plan we cannot actually execute (no gust to reach
it). It is deliberately COMPLEMENTARY to kb_draw, not overlapping.

  - prize_map (recomputed each decision): remaining = prizes we still must take; among the
    KOs reachable THIS turn (active target + gust-reachable bench) it scores each by how cheaply
    it advances the map — a single-turn FINISH (this KO takes our last prize) is the cheapest
    sequence and dominates; otherwise prefer the KO that yields the most prizes per turn while
    staying feasible. It is a coarse 1-ply map (one KO chosen now), not a full tree search.
  - feasibility GATE: a gust target we cannot actually reach with the gusting we hold (it is on
    the BENCH and we have no Boss's Orders in hand and no supporter slot) gets NO bonus — we
    never bias toward a plan we cannot execute this turn. The gate only WITHHOLDS the bonus; it
    never penalises below v1.

Why a NUDGE and not a rewrite: sabrina_v1 already emits one best-scored pick per engine select.
We ADD `PRIZEMAP_EPSILON * map_advance(o)` to a SEPARATE sort key (never mutating `scores`),
applied ONLY to options the policy already wants (base score > 0) and ONLY to ATTACK / target
CARD options. PRIZEMAP_EPSILON (default 8) is TINY vs v1's score scale (play/attack scores live
in the thousands; lethal lines score 90000) — so it can only break a TIE between options of
otherwise-equal base score, NEVER flip a magnitude decision, NEVER override lethal, NEVER
resurrect a rejected (score<=0) option, and NEVER enable an illegal pick. Flag OFF
(FMA_KB_PRIZEMAP=0) removes the term entirely => byte-equivalent behaviour to sabrina_v1.

The map module (`_prizemap_*`) references only generic engine concepts (prize counts, reachable
KOs, gust availability) and reuses the policy's own damage/gust helpers — no NEW Alakazam-only
logic — so the PLANNING shape transfers to a general "best PTCG player" engine.

Everything below the lever is sabrina_v1 VERBATIM (same deck.csv, same survival scaffolding).
...
# ── LEVER: adaptive 6-prize prize-map nudge (Knowledge OS H1b) ────────────────
# ARCHETYPE-AGNOSTIC tie-breaker. Flag-gated and reversible: OFF => byte-equivalent to v1.
FMA_KB_PRIZEMAP = os.environ.get("FMA_KB_PRIZEMAP", "1") not in ("0", "false", "False", "")
# Tiny tie-breaker weight. v1's base scores live in the thousands (lethal = 90000); this only
# separates options whose base scores are otherwise equal. Override via env to A/B-tune.
try:
    PRIZEMAP_EPSILON = float(os.environ.get("FMA_KB_PRIZEMAP_EPSILON", "8"))
except Exception:
    PRIZEMAP_EPSILON = 8.0

_DIAG = {"decisions": 0, "policy_ok": 0, "policy_fallback": 0,
         "obs_fallback": 0, "deck_returns": 0, "errors": {}, "prizemap_fires": 0}
...
class AlakazamPolicy:
    ...
    # ── LEVER: adaptive 6-prize prize-map (Knowledge OS H1b) ─────────────────
    # All methods reference only generic engine signals (prize counts, reachable KOs, gust
    # availability) + the policy's own damage/gust helpers — no NEW Alakazam-only logic.
    def _prizemap_gust_available(self):
        """Can we gust a benched opponent THIS turn (reach a non-Active target)? True if a
        Boss's Orders is in hand and the Supporter slot is still free. (Feasibility GATE
        input — H1b 'enough_gust_effects'.) Conservative: if unsure, treats gust as NOT
        available, so we never bias toward a plan we cannot execute."""
        try:
            if getattr(self.state, "supporterPlayed", False):
                return False
            return self.hand.get(C.BOSS_ORDERS, 0) >= 1
        except Exception:
            return False

    def _prizemap_remaining(self):
        """Prizes we still must take to win (the 6-prize plan's 'remaining')."""
        try:
            return len(self.me.prize)
        except Exception:
            return 6

    def _prizemap_target_advance(self, target, reachable):
        """How well KO-ing `target` (an opponent Pokémon) advances the cheapest path to the
        last prize, as a small bounded ordinal in {0,1,2,3}. Larger = keeps the map on the
        cheaper sequence. `reachable` = is this KO actually executable this turn (active, or
        gust available for a bench target)? Returns 0 (no bias) when not reachable — the
        feasibility GATE. NEVER negative => can only ever ADD to v1's score, never penalise."""
        if target is None or not reachable:
            return 0
        try:
            yield_ = prize_count(target)                       # prizes this KO yields (1/2/3)
            remaining = self._prizemap_remaining()
            # Cheapest sequence = the one that finishes in the fewest KOs. A KO that takes our
            # LAST remaining prize(s) finishes the map THIS turn -> top of the map (3).
            if yield_ >= remaining:
                return 3
            # Otherwise prefer the KO that advances the map fastest: more prizes per KO = fewer
            # KOs left = cheaper remaining sequence. 2-prize body > 1-prize body, bounded.
            if yield_ >= 2:
                return 2
            return 1
        except Exception:
    ...
    def _prizemap_bias(self, o):
        """Additive prize-map tie-breaker for ONE option. TINY vs v1's score scale, so it only
        breaks ties between otherwise-equal-base-score options; it can never flip a magnitude
        decision, override lethal, or resurrect a rejected option (applied only where base>0,
        see rank()). 0 when the flag is off or the option is not an attack / gust target."""
        if not FMA_KB_PRIZEMAP:
            return 0
        try:
            t = o.type
            # (a) ATTACK options: bias toward the attack that best advances the prize map on the
            #     current Active. (Lethal already scores 90000 in _score_attack; epsilon*3 ≈ 24
            #     is far below the 2500+ KO bonus, so ordering of real KOs is untouched except
            #     for genuine ties.)
            if t == OptionType.ATTACK:
                opp = self.opponent.active[0] if self.opponent.active else None
                if opp is None:
                    return 0
                dmg = self._alakazam_damage(o.attackId, opp)
                if dmg < getattr(opp, "hp", 1):
                    return 0                                   # not a KO this attack -> no map advance
                adv = self._prizemap_target_advance(opp, reachable=True)
                return PRIZEMAP_EPSILON * adv
            # (b) gust / switch TARGET selection (CARD option pointing at an opponent Pokémon):
            #     bias toward the target that keeps the cheapest KO sequence, gated by whether we
            #     can actually KO it now and reach it (Active = reachable; bench = need gust).
            if t == OptionType.CARD:
                ctx = self.context
                if ctx not in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
                    return 0
                if getattr(o, "playerIndex", None) != self.op_index:
                    return 0
                card = get_card(self.obs, o.area, o.index, o.playerIndex)
                if not isinstance(card, Pokemon):
                    return 0
                if self._active_best_dmg(card) < getattr(card, "hp", 1):
                    return 0                                   # we can't KO it this turn -> no advance
                is_active_target = (getattr(o, "inPlayArea", None) == AreaType.ACTIVE)
                reachable = is_active_target or self._prizemap_gust_available()
                adv = self._prizemap_target_advance(card, reachable=reachable)
                return PRIZEMAP_EPSILON * adv
            return 0
        except Exception:
            return 0
    ...
    def rank(self):
        ...
        # LEVER: prize-map tie-breaker. We build a SEPARATE sort key (scores + bias) and do NOT
        # mutate `scores` — `normalize_selection` uses `scores` with an `s > 0` wanted-threshold,
        # so leaving it untouched guarantees the nudge can only REORDER already-wanted options,
        # never resurrect a rejected one (score <= 0). The bias is also only applied to options
        # the policy already wants (base score > 0). => the lever changes ORDERING among equally
        # wanted KO targets/attacks, never WHICH cards are played, never lethal. Flag OFF =>
        # key == scores == v1 (byte-equivalent behaviour).
        try:
            opts = self.select.option
            biases = [self._prizemap_bias(opts[i]) if scores[i] > 0 else 0
                      for i in range(len(scores))]
            if any(b for b in biases):
                _DIAG["prizemap_fires"] += 1
            sort_key = [scores[i] + biases[i] for i in range(len(scores))]
        except Exception:
            sort_key = scores
        ranked = sorted(range(len(scores)), key=lambda i: sort_key[i], reverse=True)
