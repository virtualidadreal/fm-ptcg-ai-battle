"""
Sabrina KB-Draw — sabrina_v1 + ONE lever: prize-belief-aware draw guard (anti deck-out).

Provenance: byte-faithful fork of agents_official/sabrina_v1/main.py (the AlakazamPolicy,
divergence-mined vs the Elo>=1150 pool + cabt-measured). The piloting logic is kept VERBATIM
and the FMA survival scaffolding is the champion standard (repetition-safe fallbacks, final
_validate of the policy output, non-raising module load). deck.csv is byte-identical to v1.

THE ONE LEVER (KB_DRAW_GUARD): a prize-belief-aware nudge on the OPTIONAL draw decisions only
(the benched Dudunsparce Run Away Draw, and the draw/search supporters Hilda/Dawn/Poké Pad).
sabrina_v1 already preserves the deck when it is WINNING (`_deck_preserve`: big_hand + deck low).
This lever adds the OTHER half of the dominant deck-out loss mode: stop optional drawing when
(a) the deck is genuinely low AND (b) a Bayesian belief over our OWN hidden prizes says the
KEY PIECES are very likely NOT prized — i.e. we don't need to keep digging for them, so each
extra optional draw is pure deck-out risk with no upside. Hypergeometric over the unknown_pool
(deck + our own un-flipped prizes), not over a fixed 60. Derived from heurística #4 of
knowledge/synthesis/heuristicas-computables.md ("Belief-state de cartas premiadas",
hipergeométrica sobre unknown_pool). The math is verified there (vs lastlegume); we use the
direct formula, never the rounded naive percentages.

DESIGN GUARANTEES (so this stays a bounded, reversible NUDGE — not a rewrite):
  - Behind a flag: KB_DRAW_GUARD = True. Flip to False -> byte-equivalent behaviour to v1.
  - It ONLY ever turns an OPTIONAL draw OFF (returns -1). It NEVER changes attacks, evolves,
    energy attach, gusting, repositioning, or any non-draw decision. No new aggression.
  - It is STRICTLY MORE CONSERVATIVE than v1's existing `_deck_preserve`: it fires only inside
    the same low-deck danger band and only ADDS a reason to stop drawing. It can never make us
    draw MORE than v1 would. Worst case it concedes a little tempo; it cannot create an illegal
    or more reckless line.
  - The belief module `PrizeBeliefModel` is ARCHETYPE-AGNOSTIC: it takes deck/hand/discard/prize
    counts + a list of "key piece" card ids + their target counts, and answers
    `prob_all_key_pieces_safe()` via the standard hypergeometric. It carries NO Alakazam logic,
    so it is reusable for a general "best PTCG player" engine, not just cabt/Alakazam. The
    Alakazam-specific bit (which ids are "key pieces") lives in the policy, passed IN.

HONEST BASELINE (unchanged from v1): ptcg-abc's Alakazam scored ~674 on the real ladder; the
ladder is the only judge (local cabt is a filter, not a judge — cabt is anti-predictive,
Spearman -0.80). Keep sabrina_v1 (and Dragapult) as the verified floor in the other slot.
...
# ── THE LEVER (one flag, reversible) ─────────────────────────────────────────
# Flip to False -> byte-equivalent piloting to sabrina_v1 (the guard becomes a no-op).
KB_DRAW_GUARD = True

# Tunables for the guard (bounded, derived — see README). All "<=" comparisons so the band
# is a strict subset of the low-deck zone v1 already cares about.
KB_DECK_FLOOR = 9          # only consider the guard when deckCount <= this (genuinely low)
KB_SAFE_THRESHOLD = 0.85   # require >=85% belief that ALL key pieces are NOT prized to stop
                           #   digging. High bar on purpose: only stop when we're very sure we
                           #   don't NEED the deck for the win-con.


# ── archetype-AGNOSTIC prize-belief model (reusable; heurística #4) ───────────
class PrizeBeliefModel:
    """Bayesian-ish belief over our OWN hidden prizes via the standard hypergeometric.

    Pure counting — knows nothing about Alakazam (or any archetype). Inputs are counts the
    caller supplies; it answers "how likely is it that NONE of my needed key pieces is stuck
    in my un-flipped prizes". Reusable for a general PTCG agent.

    unknown_pool = cards we cannot see that could hide a key piece = deck + our own prizes
    still face-down. (Hand/discard/board are known, so a copy located there is NOT a risk.)
    """

    def __init__(self, deck_count, prizes_remaining, known_counts):
        # known_counts[card_id] = copies of that card we have ALREADY LOCATED (hand+board+
        # discard) and therefore are NOT prized.
        self.deck_count = max(0, int(deck_count))
        self.prizes_remaining = max(0, int(prizes_remaining))
        self.known_counts = known_counts or {}
        self.unknown_pool = self.deck_count + self.prizes_remaining

    @staticmethod
    def prob_at_least_one_prized(copies_unaccounted, prizes_remaining, unknown_pool):
        """P(>=1 of `copies_unaccounted` is among the `prizes_remaining` face-down prizes),
        drawing the prize set uniformly from the `unknown_pool`. Heurística #4, exact form
        (verified vs lastlegume). Hypergeometric, NOT a fixed-60 approximation."""
        c = max(0, int(copies_unaccounted))
        pr = max(0, int(prizes_remaining))
        up = max(0, int(unknown_pool))
        if c <= 0 or pr <= 0 or up <= 0:
            return 0.0
        if c >= up:                      # every unseen card is a copy -> certainly prized
            return 1.0
        if pr >= up:                     # all unseen cards are prizes -> certainly prized
            return 1.0
        try:
            return 1.0 - comb(up - c, pr) / comb(up, pr)
        except Exception:
            return 0.0

    def prob_piece_safe(self, card_id, target_count):
        """P(we are NOT missing a needed copy of `card_id` to the prizes).
        target_count = how many copies we actually need in play/hand for the win-con.
        copies_unaccounted = needed copies we have NOT yet located."""
        located = int(self.known_counts.get(card_id, 0))
        unaccounted = max(0, int(target_count) - located)
        if unaccounted <= 0:
            return 1.0                   # we already hold/played enough copies -> safe
        p_prized = self.prob_at_least_one_prized(
            unaccounted, self.prizes_remaining, self.unknown_pool)
        return 1.0 - p_prized

    def prob_all_key_pieces_safe(self, key_pieces):
        """key_pieces = [(card_id, target_count), ...]. Product of per-piece safe probs
        (independent-ish approximation — fine for a bounded nudge gate)."""
        p = 1.0
        for cid, need in key_pieces:
            p *= self.prob_piece_safe(cid, need)
        return p
...
_DIAG = {"decisions": 0, "policy_ok": 0, "policy_fallback": 0,
         "obs_fallback": 0, "deck_returns": 0, "errors": {}, "kb_guard_fires": 0}
...
class AlakazamPolicy:
    ...
    # ── THE LEVER: prize-belief-aware draw guard (archetype-agnostic model, Alakazam keys) ──
    def _kb_key_pieces(self):
        """The win-con pieces this deck needs in PLAY to keep KO-ing. Archetype-specific list
        lives HERE (passed INTO the generic model). For Alakazam: at least one Alakazam-line
        attacker (Alakazam 743) and the draw engine (Dudunsparce 66). target_count=1 each —
        we only need one of each online to close. Reusable shape: [(card_id, need), ...]."""
        return [(C.ALAKAZAM, 1), (C.DUDUNSPARCE, 1)]

    def _kb_known_counts(self):
        """Copies of each key piece we have already LOCATED (hand + board + discard) and that
        therefore CANNOT be prized. (Discard counts as 'not prized' — it's a known zone.)"""
        kc = {}
        for cid, _need in self._kb_key_pieces():
            kc[cid] = self.field.get(cid, 0) + self.hand.get(cid, 0) + self.discard.get(cid, 0)
        return kc

    def _kb_draw_guard_active(self):
        """Should the prize-belief guard STOP an optional draw this decision?

        Fires only when ALL hold (strict subset of v1's low-deck danger zone):
          1) flag on,
          2) the deck is genuinely low (deckCount <= KB_DECK_FLOOR),
          3) we ALREADY have an attacker that can act (so digging is not how we win now),
          4) the Bayesian belief that ALL key pieces are NOT prized is high
             (>= KB_SAFE_THRESHOLD) -> we don't NEED to keep drawing for the win-con,
             so each optional draw is pure deck-out risk.

        It NEVER fires while we still need to dig (low belief => key piece likely prized or not
        yet located => keep v1's aggressive draw). So it can only ever make us draw LESS, never
        more, and only when continuing to draw has no win-con upside."""
        if not KB_DRAW_GUARD:
            return False
        try:
            if self.me.deckCount > KB_DECK_FLOOR:
                return False
            if not self._have_attacker():
                return False
            model = PrizeBeliefModel(
                deck_count=self.me.deckCount,
                prizes_remaining=len(self.me.prize),
                known_counts=self._kb_known_counts(),
            )
            p_safe = model.prob_all_key_pieces_safe(self._kb_key_pieces())
            return p_safe >= KB_SAFE_THRESHOLD
        except Exception:
            return False              # any belief-math hiccup -> behave exactly like v1
    ...
    def _score_ability(self, o):
        ...
        if card.id == C.DUDUNSPARCE:
            ...
            if self._deck_preserve():
                return -1
            # THE LEVER: prize-belief-aware deck-out guard. ON the OPTIONAL benched-copy draw
            # only (pure filtering). When the deck is low and belief says the win-con pieces are
            # NOT prized (we don't need to dig), stop the optional Run Away Draw. Strictly more
            # conservative than _deck_preserve above (which needs a winning hand NOW); this
            # catches the case where we have an attacker but not yet a lethal hand, deck is low,
            # and digging only risks deck-out for pieces we already hold.
            if self._kb_draw_guard_active():
                _DIAG["kb_guard_fires"] += 1
                return -1
    ...
    def _score_play_trainer(self, card):
        ...
        if cid in (C.HILDA, C.DAWN, C.POKE_PAD) and self._deck_preserve():
            return -1
        # THE LEVER (same guard, draw/search supporters): when deck is low and belief says the
        # win-con pieces are NOT prized, don't burn a draw/search supporter into the deck-out.
        # NB: we DON'T suppress when draw_for_ko is true — if drawing this turn reaches lethal on
        # the Active, that draw has clear upside and overrides the guard (no tempo loss on a kill).
        if cid in (C.HILDA, C.DAWN, C.POKE_PAD) and not draw_for_ko and self._kb_draw_guard_active():
            _DIAG["kb_guard_fires"] += 1
            return -1
        if cid == C.HILDA:
