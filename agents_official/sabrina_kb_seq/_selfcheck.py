"""Self-check for Sabrina kb_seq (turn-sequencing nudge): run the agent vs itself in cabt.
...
sequencing nudge does not crash and never emits an INVALID / fallback.

PASS requires: completed == N_GAMES, 0 None-rewards, 0 policy/obs fallbacks, deck == 60.

It also INSTRUMENTS how often the sequencing bias is non-zero (monkeypatch on the policy
...
# ── sequencing-bias firing instrumentation ───────────────────────────────────
SEQ = {"calls": 0, "fired": 0, "bias_sum": 0.0}
_orig = agent_mod.AlakazamPolicy._seq_bias
...
agent_mod.AlakazamPolicy._seq_bias = _wrapped


def main():
    print("kaggle_environments:", getattr(ke, "version", "?"))
    print("deck loaded ids:    ", len(getattr(agent_mod, "my_deck", [])))
    print("FMA_KB_SEQ flag ON: ", agent_mod.FMA_KB_SEQ)
    print("SEQ_EPSILON:        ", agent_mod.SEQ_EPSILON)
    ...
    print("-" * 60)
    print(f"seq bias calls:   {SEQ['calls']}  (every scored option that hit _seq_bias)")
    print(f"seq bias NON-ZERO:{SEQ['fired']}  (times it returned != 0)")
    print(f"seq bias sum:     {SEQ['bias_sum']}")
    print("=" * 60)
