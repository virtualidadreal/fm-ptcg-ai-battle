"""
Sabrina kb_seq — Sabrina v1 + ONE lever: turn SEQUENCING (Knowledge OS heuristic #5).

================================ THE ONE LEVER ================================
This is a single-variable fork of `sabrina_v1`. The ONLY behavioural change is a
tiny, flag-gated, ARCHETYPE-AGNOSTIC phase-ordering NUDGE added to the option score:
when several legal actions are offered together, prefer the EARLIER turn phase
(draw before search, attach energy as the LAST setup action before attacking,
disruption before revealing strength). It only REORDERS legal actions — it never
changes WHICH cards are played, never enables an illegal pick, and is fully
reversible (flag OFF => byte-equivalent behaviour to v1).

Source heuristic: knowledge/synthesis/heuristicas-computables.md §5 "Ordenar el turno
por fase + minimizar commitment irreversible" (sequencing, P1). The doc models the turn
as a queue ordered by phase ordinal, tie-broken by EV:
    PHASE = {draw_refresh:0, disruption:1, directed_search:2,
             reversible_setup:3, attach_energy_or_tool:4, attack:5}
    order_turn(actions) = sorted(actions, key=lambda a: (phase(a), -EV(a)))
The doc's own caveat: this method does NOT rotate with the meta (it is mechanics, not a
card list), but the phase PREDICATES must be built from the cabt option model — which is
exactly what `_seq_phase()` below does: it maps the engine's generic OptionType to a phase
...
Why a NUDGE and not a hard reorder: sabrina_v1 already emits one best-scored pick per
engine select, and the engine drives phases by re-asking. So instead of replacing the
selection loop, we ADD `SEQ_EPSILON * (PHASE_LATE - phase_ordinal)` to each option's
score: earlier phases get a hair more, later phases a hair less. SEQ_EPSILON (default 8)
is deliberately TINY relative to v1's score scale (card/play scores live in the thousands,
attack scores ~1000–90000), so it can only break a TIE between options of otherwise-equal
base score (e.g. two playable items, or play-vs-attach when both look equal) — it can never
flip a real magnitude decision, never override a KO/lethal line, and never make the agent
play a worse card. Disabling the flag removes the term entirely => v1 behaviour.
...
# ── LEVER: turn-sequencing nudge (Knowledge OS heuristic #5) ──────────────────
# ARCHETYPE-AGNOSTIC. This whole block references only generic engine concepts
# (OptionType / SelectContext) — no Sabrina/Alakazam card ids — so it is reusable
# by any rule-based cabt pilot. Flag-gated and reversible.
FMA_KB_SEQ = os.environ.get("FMA_KB_SEQ", "1") not in ("0", "false", "False", "")
# Tiny tie-breaker weight. v1's base scores live in the thousands; SEQ_EPSILON only
# separates options whose base scores are otherwise equal. Override via env to A/B-tune.
try:
    SEQ_EPSILON = float(os.environ.get("FMA_KB_SEQ_EPSILON", "8"))
except Exception:
    SEQ_EPSILON = 8.0

# Phase ordinal per the heuristic: lower = should happen EARLIER in the turn.
#   draw_refresh:0  disruption:1  directed_search:2  reversible_setup:3
#   attach_energy_or_tool:4  attack:5
# Built from the engine's OptionType, NOT from any card list (so it transfers).
PHASE_LATE = 5            # max ordinal (attack); used so earlier phases score higher
_SEQ_PHASE_BY_OPTYPE = {
    # draw / revealing-to-self refresh first (abilities like Run Away Draw, draw plays
    # resolve through PLAY/ABILITY — see _seq_phase for the PLAY refinement)
    OptionType.ABILITY: 0,
    # reversible setup (evolve, bench placement) before committing resources
    OptionType.EVOLVE: 3,
    # attach energy / tool = LAST setup action before the attack (minimise commitment)
    OptionType.ENERGY: 4,
    OptionType.ATTACH: 4,
    OptionType.TOOL_CARD: 4,
    OptionType.ENERGY_CARD: 4,
    # retreat is a reposition right before attacking
    OptionType.RETREAT: 4,
    OptionType.ATTACK: 5,
    # END is terminal — leave it at the latest phase so any real action is preferred
    OptionType.END: 5,
}
...
class AlakazamPolicy:
    ...
    def rank(self):
        ...
        # LEVER: sequencing tie-breaker. We build a SEPARATE sort key (scores + bias) and
        # do NOT mutate `scores` itself — `normalize_selection` uses `scores` with an `s > 0`
        # wanted-threshold, so leaving it untouched guarantees the nudge can only REORDER
        # already-wanted options, never resurrect a rejected one (score <= 0). The bias is
        # also only applied to options the policy already wants (base score > 0). => the lever
        # changes ordering, never WHICH cards are played. Flag OFF => key == scores == v1.
        try:
            opts = self.select.option
            sort_key = [
                (scores[i] + (self._seq_bias(opts[i]) if scores[i] > 0 else 0))
    ...
    # ── LEVER: sequencing phase classification (archetype-agnostic) ──────────
    def _seq_phase(self, o):
        """Phase ordinal for option `o` per Knowledge OS heuristic #5. Lower = earlier
        in the turn. Uses ONLY generic engine signals (OptionType + card TEXT/cardType),
        never a Sabrina/Alakazam card id — so this is reusable by any cabt pilot.

        Returns None when the option type has no defined phase (then no bias is applied,
        leaving v1's score untouched for that option)."""
        t = o.type
        if t == OptionType.PLAY:
            # Refine the PLAY phase from the card's GENERIC role (text/type), so the
            # three draw/disruption/search principles fire without any card-id list:
            #   draw refresh (0) -> directed search (2) -> other reversible plays (3),
            #   with disruption/gusting (1) sitting between draw and search.
            try:
            ...
            if d.cardType == CardType.POKEMON:
                return 3                       # benching a basic = reversible setup
            txt = " ".join((s.text or "") for s in (d.skills or [])).lower()
            if "draw" in txt and "search" not in txt:
                return 0                       # Principle 1: draw/refresh before search
            is_gust = ("switch" in txt or "active spot" in txt) and "opponent" in txt
            if is_gust:
                return 1                       # disruption / gust before revealing strength
            if "search your deck" in txt or "look at" in txt:
                return 2                       # directed search after draw
            return 3                           # other trainers = reversible setup
        return _SEQ_PHASE_BY_OPTYPE.get(t, None)

    def _seq_bias(self, o):
        """Additive sequencing tie-breaker: earlier phases get a hair more score, later
        phases a hair less. TINY vs v1's score scale => only breaks ties, never overrides
        a real magnitude decision. 0 when the flag is off or the phase is undefined."""
        if not FMA_KB_SEQ:
            return 0
        ph = self._seq_phase(o)
        if ph is None:
            return 0
        return SEQ_EPSILON * (PHASE_LATE - ph)
